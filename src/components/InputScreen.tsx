...
function InputScreen({}: Props) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setData(json ?? null);
      // Initialize AdSense ads after data is loaded
      initializeAdSense();
      setError("");
    } catch (error: any) {
  ...
  const initializeAdSense = () => {
    ...
    if (!document.getElementById("adsense-script")) {
      ...
      script.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-4342939946293194";
  ...
  const downloadVideo = async (videoUrl: string, filename: string, quality: string) => {
    ...
    try {
      ...
      const externalServiceUrl = `https://dll.help-tiktokio-cam.workers.dev/api/download?url=${encodeURIComponent(videoUrl)}&type=.mp4&title=${encodeURIComponent(filename)}_${quality}`;
